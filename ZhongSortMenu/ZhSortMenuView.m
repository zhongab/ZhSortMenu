...
@interface ZhSortMenuView()<UITableViewDataSource,UITableViewDelegate>{
    struct {
...
@property (nonatomic, assign) CGFloat rightTableWidth;
@end

@implementation ZhSortMenuView
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height{
    _leftTableWidth = SCREEN_WIDTH / 2;
    
   return [self initWithOrigin:origin andHeight:height leftWidth:_leftTableWidth];
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height leftWidth:(CGFloat)leftWidth{
    ...
    if (self) {
        ...
        _leftTableWidth = leftWidth;
        _rightTableWidth = SCREEN_WIDTH - _leftTableWidth;
        
        //初始化两个tableView
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y + self.frame.size.height, self.leftTableWidth, 0) style:UITableViewStylePlain];
        ...
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + _leftTableView.frame.origin.x + self.leftTableWidth, origin.y + self.frame.size.height, self.rightTableWidth, 0) style:UITableViewStylePlain];
...
- (void)setDataSource:(id<ZhSortMenuDataSource>)dataSource{
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:0 inColumn:i] > 0 && _dataSourceFlag.numberOfItemsInRow) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:i row:0 item:0]];
        }else {
            titleString = [_dataSource menu:self titleForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:i row:0]];
        }
...
- (void)animateTableView:(UITableView *)tableview show:(BOOL)show complete:(complete)complete {
    ...
    if (show) {
        if (haveItems) {
            _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height,self.leftTableWidth, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + _leftTableView.frame.origin.x + self.leftTableWidth, self.origin.y +self.height, self.rightTableWidth, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, self.leftTableWidth, tableViewHeight);
                _rightTableView.frame = CGRectMake(self.origin.x + _leftTableView.frame.origin.x + self.leftTableWidth, self.origin.y +self.height, self.rightTableWidth, tableViewHeight);
            }else {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlag.numberOfItemsInRow) {
            ...
            NSInteger count = [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn];
            return count;
        }else {
            return 0;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            if (_dataSourceFlag.imageNameForRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageNameForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            ...
            if (_dataSourceFlag.detailTextForRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            }else {
        ...
        if (_isShowCellArrow && _dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:_currentSelectedColumn] > 0) {
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            ...
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            
            if (_dataSourceFlag.imageNameForItemInRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    cell.imageView.image = [UIImage imageNamed:imgName];
                }else {
                    cell.imageView.image = nil;
            ...
            if (_dataSourceFlag.detailTextForItemInRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            }else {
                cell.detailTextLabel.text = nil;
            }
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _leftTableView) {
        ...
        if (isClickHaveItemValid && _delegate &&[_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        }
    }else {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:[_currentSelectedRows[_currentSelectedColumn] integerValue] item:indexPath.item]];
        }
    }
}
...
- (void)selectDeafultIndexPath {
    [self selectIndexPath:[ZhIndexPath indexPathWithColumn:0 row:0]];
}
//获取IndexPath所对应的字符串
- (NSString *)titleForRowAtIndexPath:(ZhIndexPath *)indexPath {
...
- (void)selectIndexPath:(ZhIndexPath *)indexPath {
    ...
    if (indexPath.item < 0 ) {
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
            [_delegate menu:self didSelectRowAtIndexPath:[ZhIndexPath indexPathWithColumn:indexPath.column row:indexPath.row item:0]];
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
...
- (BOOL)setMenuWithSelectedRow:(NSInteger)row {
    ...
    if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn] > 0) {
        if (_isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:row]];
...
- (void)setMenuWithSelectedItem:(NSInteger)item {
    ...
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZhIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedMenudRow item:item]];
    [self animateIndicator:_indicators[_currentSelectedColumn] background:_backGroundView tableView:_leftTableView title:title reverse:NO complecte:^{
        _isShow = NO;
    }];
}
